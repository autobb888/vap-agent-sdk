...
class VAPAgent extends node_events_1.EventEmitter {
    ...
    async registerWithVAP(agentData) {
        ...
        try {
            ...
            if (regErr instanceof Error && 'statusCode' in regErr && regErr.statusCode === 409) {
    ...
    async registerCanaryToken() {
        try {
            ...
            console.warn('[VAP Agent] Canary registration failed (non-fatal):', e.message);
        }
    }
    /**
     ...
     */
    async setRevokeRecoverAuthorities(revokeAddress, recoverAddress) {
        if (!this.wif || !this.iAddress) {

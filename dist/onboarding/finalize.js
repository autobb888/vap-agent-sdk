...
async function resolveProfile(mode, profile, hooks) {
    ...
    const nameErr = (0, validation_js_1.validateAgentName)(name);
    if (nameErr)
        throw new Error(nameErr);
    const typeRaw = await prompt(`Agent type (${validation_js_1.VALID_TYPES.join('|')})`, 'autonomous');
    const typeErr = (0, validation_js_1.validateAgentType)(typeRaw);
    if (typeErr)
        throw new Error(typeErr);
    const type = typeRaw;
    const description = await prompt('Description (10-1000 chars)');
    const descErr = (0, validation_js_1.validateDescription)(description);
    if (descErr)
        throw new Error(descErr);
    // Optional fields
    ...
    const protocolsRaw = await prompt(`Protocols (comma-separated: ${validation_js_1.VALID_PROTOCOLS.join(',')}) (optional)`);
    // Parse tags
    let tags;
    if (tagsRaw) {
        tags = tagsRaw.split(',').map(t => t.trim()).filter(Boolean);
        const tagsErr = (0, validation_js_1.validateTags)(tags);
        if (tagsErr)
            throw new Error(tagsErr);
    }
    // Validate website
    if (website) {
        const urlErr = (0, validation_js_1.validateUrl)(website);
        if (urlErr)
            throw new Error(`Website: ${urlErr}`);
    }
    // Validate avatar
    if (avatar) {
        const urlErr = (0, validation_js_1.validateUrl)(avatar);
        if (urlErr)
            throw new Error(`Avatar: ${urlErr}`);
    }
    // Parse protocols
    let protocols;
    if (protocolsRaw) {
        const parsed = protocolsRaw.split(',').map(p => p.trim()).filter(Boolean);
        const protErr = (0, validation_js_1.validateProtocols)(parsed);
        if (protErr)
            throw new Error(protErr);
        protocols = parsed;
    }
    ...
    if (addSession === 'y' || addSession === 'yes') {
        ...
        if (allowedFileTypesRaw) {
            ...
            if (types.length > 0)
                s.allowedFileTypes = types;
        }
        if (Object.keys(s).length > 0) {
            const sessionErr = (0, validation_js_1.validateSessionInput)(s);
            if (sessionErr)

...
class WebhookAgent extends node_events_1.EventEmitter {
    client;
    keypair;
    wif;
    networkType;
    ...
    constructor(config) {
        ...
        this.keypair = (0, keypair_js_1.keypairFromWIF)(config.wif, this.networkType);
    ...
    async login() {
        ...
        this.loginPromise = (async () => {
            try {
                ...
                if (!identity)
                    throw new Error('identityName or iAddress required for login');
                const { challenge, challengeId } = await this.client.getAuthChallenge();
                const signature = (0, signer_js_1.signMessage)(this.wif, challenge, this.networkType);
                ...
                if (!res.ok) {
                    ...
                    throw new Error(`Login failed: ${err?.error?.message || res.statusText}`);
                }
    ...
    async handleWebhook(payload) {
        ...
        try {
            ...
            return { handled: false, error: `Login failed: ${err.message}` };
        }
        try {
            switch (event) {
                case 'job.requested':
    ...
    async deliverJob(jobId, deliveryContent) {
        await this.login();
        const job = await this.client.getJob(jobId);
        const timestamp = Math.floor(Date.now() / 1000);
    ...
    async handleJobRequested(jobId) {
    ...
    async handleJobDisputed(jobId, data) {
        if (this.handler?.onJobDisputed) {
            const job = await this.client.getJob(jobId);
            await this.handler.onJobDisputed(job, data?.reason || 'Unknown');
        }
        return { handled: true, jobId, action: 'disputed' };
    }
    async handleJobCancelled(jobId, data) {
        if (this.handler?.onJobCancelled) {
            const job = await this.client.getJob(jobId);
            await this.handler.onJobCancelled(job, data?.reason);
        }
        return { handled: true, jobId, action: 'cancelled' };
    }
    async handleSessionEnding(jobId, data) {
        if (this.handler?.onSessionEnding) {
            const job = await this.client.getJob(jobId);
            await this.handler.onSessionEnding(job, data?.reason || '', data?.requestedBy || '');
